...
public class AuctionController {
	...
	public String moveToMyBIdList(@RequestParam(value="page", defaultValue="1") int currentPage, Model model,
		...
		// "successBid" 상태일 경우 낙찰된 항목만 필터링 - 위에서 애초에 종료된 것들만 들고와 절약
		if (aucStatus != null && aucStatus.equals("successBid")) {
			// HashMap을 사용하여 경매 번호를 기준으로 중복을 제거하고 가격이 높은 항목만 유지할 예정
	        Map<Integer, Auction> filteredBidMap = new LinkedHashMap<>();
	        Map<Integer, BiddingDetail> filteredDetailMap = new LinkedHashMap<>();

	        for (Auction auction : myBidList) {
	            for (BiddingDetail detail : detailList) {
	                if (auction.getAucNo() == detail.getAucNo() && auction.getAucFinishPrice() == detail.getBidPrice()) {
	                    if (!filteredBidMap.containsKey(auction.getAucNo()) || auction.getAucFinishPrice() > filteredBidMap.get(auction.getAucNo()).getAucFinishPrice()) {
	                        // 순환될 때 없는 경매라면 map에 집어넣고, 있는 경매라면 값을 덮어씌운다
	                        filteredBidMap.put(auction.getAucNo(), auction);
	                        filteredDetailMap.put(detail.getAucNo(), detail);
	                    }
	                }
	            }
	        }
	        
	        // Map을 ArrayList에 넣어줌 - 중복이 제거된 최고값들만 들어가 있음
	        myBidList = new ArrayList<>(filteredBidMap.values());
	        detailList = new ArrayList<>(filteredDetailMap.values());
	        
	        
	        //sort 값에 따라서 값을 뒤집어 준다
	        if (sort != null && sort.equals("low")) {
	            myBidList.sort(Comparator.comparing(Auction::getAucFinishPrice));
	            detailList.sort(Comparator.comparing(BiddingDetail::getBidPrice));
	        } else if (sort != null && sort.equals("high")) {
	            myBidList.sort(Comparator.comparing(Auction::getAucFinishPrice).reversed());
	            detailList.sort(Comparator.comparing(BiddingDetail::getBidPrice).reversed());
	        }
	    }
